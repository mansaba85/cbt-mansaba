...
export default function Proctoring() {
  ...
  const fetchLiveProctoring = async () => {
    try {
      ...
      // Prevent selection issues by re-checking group
      if (data.length > 0 && selectedGroup === 'all' && false) {
         // Maybe let selectedGroup remain what user selected
      }
      setStudents(data);
    } catch(e) { }
  };
  ...
  // Set default kelas ke grup paling pertama supaya tidak "Membuka Semua" (menghindari hang/berat di browser jika 1000 siswa)
  const [selectedGroup, setSelectedGroup] = useState<string>(availableGroups.length > 0 ? availableGroups[0] : 'all');

  const activeCount = students.filter(s => s.group === selectedGroup && (s.status === 'online' || s.status === 'warning')).length;
  const finishedCount = students.filter(s => s.group === selectedGroup && s.status === 'finished').length;
  const lockedCount = students.filter(s => s.group === selectedGroup && s.status === 'locked').length;
  const totalCount = students.filter(s => s.group === selectedGroup).length;
  ...
  const handleAction = async (studentId: string, action: 'unlock' | 'logout' | 'reset_warnings' | 'reset_finished' | 'add_time' | 'toggle_cheat_exempt') => {
    ...
    try {
      ...
      fetchLiveProctoring(); // Immediate refresh after action
    } catch (e) {
  ...
  const filteredStudents = students.filter(s => {
    // 1. Wajib filter berdasarkan grup (kecuali pilih "Semua Kelas" asalkan kuat memory-nya)
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
         {paginatedStudents.map(student => {
            ...
            return (
              <div key={student.id} className={`p-4 rounded-2xl border transition-all relative ${cardBg}`}>
