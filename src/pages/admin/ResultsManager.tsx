...
const ResultsManager: React.FC = () => {
  ...
  const handleBulkRegrade = async () => {
    ...
    try {
      ...
      toast.error("Beberapa data gagal dikalkulasi");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const exportToExcel = () => {
    ...
    const colWidths = [
      { wch: 5 }, { wch: 20 }, { wch: 10 }, { wch: 20 }, { wch: 15 },
  ...
  const exportToPDF = () => {
    // Memanggil print browser native yang bisa diarahkan ke 'Save as PDF'
    window.print();
  };
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 pb-20">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-slate-100 italic">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Waktu Mulai</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Durasi</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Test</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">User</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Nama Peserta</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Group</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Poin</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Skor (%)</th>
            ...
            <tbody className="divide-y divide-slate-50 italic">
              {selectedExam === '' ? (
                ...
                <tr key={res.id} className={`transition-colors group ${selectedIds.includes(res.id) ? 'bg-indigo-50/50' : 'hover:bg-blue-50/30'}`}>
                  ...
                  <td className="px-6 py-4 text-[12px] font-bold text-slate-500">{res.duration}</td>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mt-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
             ...
             <button className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold rounded shadow-sm transition-colors shrink-0">
               Kunci
             </button>
             <button className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold rounded border border-slate-200 shadow-sm transition-colors shrink-0">
               Buka Kunci
             </button>
             <button className="px-4 py-2 text-white text-sm font-bold rounded shadow-sm transition-colors shrink-0" style={{ backgroundColor: '#7a3e8c' }}>
               +5 Min
             </button>
          </div>
        </div>
      ...
      {/* Export Action Bar */}
      <div className="mt-8 flex flex-col md:flex-row items-center justify-between bg-slate-50 border border-slate-200 p-5 rounded-2xl print:hidden">
        <div>
